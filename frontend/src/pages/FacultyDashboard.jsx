...
export default function FacultyDashboard() {
  const { user } = useAuth();
  const { subjects, courseName } = useData();
  const { push } = useToast();
  const [notices, setNotices] = useState(null);
  const mySubjects = subjects.filter((s) => s.faculty === user.linkedId);
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden glass rounded-2xl p-5 sm:p-6 shadow-sm">
        <div className="relative z-10">
          ...
          <p className="text-sm text-slate-500 mt-1">You're teaching {mySubjects.length} subject{mySubjects.length !== 1 ? "s" : ""} this semester.</p>
        </div>
      </motion.div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <QuickLink to="/faculty/attendance" icon={CalendarCheck} color="#0891b2" label="Mark Attendance" delay={0} />
        <QuickLink to="/faculty/marks" icon={Award} color="#2563eb" label="Enter Marks" delay={0.05} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {mySubjects.map((s, i) => (
            <motion.div key={s.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
              ...
              <div className="text-xs text-slate-400 mt-0.5">{courseName(s.course)} · Semester {s.semester} · {s.credits} credits</div>
            </motion.div>
          ))}
          {mySubjects.length === 0 && <p className="text-sm text-slate-400 col-span-2">No subjects assigned yet.</p>}
        </div>
      </TiltCard>
